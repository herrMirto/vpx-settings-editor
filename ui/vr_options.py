...
def load_vr_options(main_window):
    ...
    manager.load_options(
        "PlayerVR",
        VR_OPTIONS,
        checkbox_options={"CaptureExternalDMD", "CapturePUP"},
        defaults=DEFAULTS
    )

    # Carregar também as opções do Player
...
def prepare_vr_updates(main_window):
    manager = WidgetOptionManager(main_window)
    ...
    # Também preparar Player Options
    updates["Player"] = {}
    for vr_option, player_option in PLAYER_OPTIONS.items():
        widget = getattr(main_window.ui, vr_option, None)
        if widget:
            updates["Player"][player_option] = "1" if widget.isChecked() else "0"

    return updates

def on_save_vr_clicked(main_window):
    updates = prepare_vr_updates(main_window)
    if show_diff_table(ini, updates, parent=main_window):
        ...
        try:
            ...
            logger.error(f"Error saving VR Options: {e}")
