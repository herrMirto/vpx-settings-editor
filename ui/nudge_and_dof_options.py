...
NUDGE_OPTIONS = [
    "LRAxis", "LRAxisFlip", "UDAxis", "UDAxisFlip", "PBWEnabled", "PBWNormalMount",
    "PBWRotationCB", "PBWRotationvalue", "PBWAccelGainX", "PBWAccelGainY", "PBWAccelMaxX",
    "PBWAccelMaxY", "EnableNudgeFilter", "EnableLegacyNudge", "LegacyNudgeStrength",
    "AccelVelocityInput", "TiltSensCB", "TiltSensValue", "PlungerAxis", "PlungerSpeedScale",
    "ReversePlungerAxis", "DeadZone", "PlungerRetract", "EnableMouseInPlayer",
    "EnableCameraModeFlyAround", "InputApi", "RumbleMode", "JoyPMBuyIn", "JoyPMCoin3",
    "JoyPMCoin4", "JoyPMCoinDoor", "JoyPMCancel", "JoyPMDown", "JoyPMUp", "JoyPMEnter"
]

DOF_OPTIONS = [
    "ForceDisableB2S", "DOFContactors", "DOFKnocker", "DOFChimes", "DOFBell", "DOFGear",
    "DOFShaker", "DOFFlippers", "DOFTargets", "DOFDroptargets", "DOFPlugin"
]
...
def load_nudge_dof_options(main_window):
    ...
    for section, options in NUDGE_DOF_OPTIONS.items():
        manager.load_options(
            section,
            options,
            defaults={**DEFAULTS, **DOF_DEFAULTS}
        )
    logger.info("=== Nudge and DOF Options loaded ===")

def prepare_nudge_dof_updates(main_window):
    manager = WidgetOptionManager(main_window)
    updates = {}
    for section, options in NUDGE_DOF_OPTIONS.items():
        updates.update(manager.prepare_updates(
            section,
            options
        ))
    return updates

def on_save_nudge_dof_clicked(main_window):
    updates = prepare_nudge_dof_updates(main_window)
    if show_diff_table(ini, updates, parent=main_window):
        ...
        try:
            ...
            logger.error(f"Error saving Nudge and DOF Options: {e}")
